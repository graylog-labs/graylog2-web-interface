class Message
  ...
  def self.all_of_hostgroup hostgroup, page
    ...
    return self.all :limit => LIMIT, :order => "_id DESC", :conditions => { :host => { "$in" => hostgroup.get_hostnames }, :deleted => false }, :offset => self.get_offset(page), :fields => { :full_message => 0 }
  end
  ...
  def self.count_of_hostgroup hostgroup
    return self.count :conditions => { :host => { "$in" => hostgroup.get_hostnames }, :deleted => false }
  end
